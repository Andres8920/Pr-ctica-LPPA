...
function validaApellido() {
    ...
    if (!elemento.checkValidity()) {
        ...
        if (elemento.validity.patternMismatch) {
            error2(elemento, "El apellido debe tener entre 2 y 15 caracteres");
        }
        //error(elemento);
        return false;
    }
...
function validaEmail() {
    ...
    if (!elemento.checkValidity()) {
        ...
        if (elemento.validity.patternMismatch) {
            error2(elemento, "El email debe tener entre 1 y 100 letras");
        } //error(elemento);
        return false;
    }
    return true;
...
function validar(e) {
    borrarError();
    if (validaNombre() && validaApellido() && validaEdad() && validaEmail() && confirm("Pulsa aceptar si deseas enviar el formulario")) {
        return true
    } else {
        e.preventDefault();
        return false;
